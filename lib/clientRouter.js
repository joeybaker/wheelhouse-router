...
router.start = function start(immediate){
  ...
  if (Backbone.History.started){
    // if we've already started, and no routes matched, check to see if any of the newly added routes match the current URL
  ...
  // if we've already decided to start, don't try to start again. This will prevent two routers from conflicting start statements since start is async by default
  else if (Backbone.history._starting){
    // assume that the previous start will look at the routes we've added in this router b/c we got in a start before Backbone.history kicked off
    this._started = true
    return this
  }
...
router._parseRoutes = function(options){
  ...
  _.each(routes, function(actions, path){
    ...
    _.each(actions, function(controllerAction, method){
      ...
      if (_.keys(this).concat(['render', 'start']).indexOf(fn) > -1 || fn.charAt(0) === '-')
...
router._setCollection = function(collectionName, collectionData, options){
  ...
  if (this.options.app.Datas[collectionName])
    ...
    this.options.app.Datas[collectionName] = new this.options.app.Collections[collectionName](_.isArray(collectionData) ? collectionData : collectionData == null ? [] : [collectionData] , options)
...
router._fetchCollection = function(collection, callback, options){
  ...
  if (options.options.url) collection.url = options.options.url
...
router._getViewData = function(opts, collection){
  var result = {}
