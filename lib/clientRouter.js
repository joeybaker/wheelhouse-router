...
module.exports = Backbone.Router.extend({
    ...
    _.each(routes, function(actions, path){
      _.each(actions, function(controllerAction, method){
        var self = this
          ...
          , pathParsed = (path.charAt(0) === '/' ? path.substring(1) : path).replace('*', '*splat').replace(/\(\/\)|\//, '') + '(/)'
  ...
  , render: function(view, collection, options){
    var collectionData = !this._started ? window._bootstrapData : null
      , done = function(coll){
        this._setView(view, coll, options).render()
      }

    if (!collection) done.call(this)
    else this._fetchCollection(this._setCollection(collection, collectionData), done.bind(this))
