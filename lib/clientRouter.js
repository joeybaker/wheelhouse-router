  ...
  if (Backbone.History.started){
    ...
    */
    if (!this._started && Backbone.history.urlMatch() && !this.options.app._currentFragment){
      if (Backbone.history.loadUrl()) this._started = true
    }
...
router._ctx = {
  res: {
    // path and code to match server-side syntax
    redirect: function redirect(path, code, options){
      // if no options are specified, assume we want to trigger this route
      // navigate will return false if it failed
      return this.navigate(path, options || {trigger: true})
    }
  }
  , req: {
    user: {}
...
router._getCtx = function _getCtx(options){
  var ctx = this._ctx

  options = _.defaults(options || {}, this.options)

  ctx.req.user = options.user.model
    ? new options.user.model(window[options.user.key])
    : window[options.user.key]

  ctx.res.redirect = ctx.res.redirect.bind(this)
...
router._addRoutes = function(options){
  _.each(options.routes, function(actionName, route){
    this.route(route, actionName, function(){
      var params = options.actions[actionName].bind(this._getCtx())
      this.render(params.view, params.collection, params)
    }.bind(this))
  }, this)
}
...
router._parseRoutes = function(options){
  ...
  _.each(routes, function(actions, path){
    ...
    _.each(actions, function(controllerAction, method){
      ...
      if (method === 'get'){
        this.route(pathParsed, controllerAction, function(){
          var params = controller[fn].apply(self._getCtx(), arguments)
...
router._updateRender = function(renderName){
  ...
  if (this.options.app._currentRender
    && this.options.app._currentRender.cid !== this.options.app.Renders[renderName].cid){
    ...
    this.options.app._currentRender._rendered = false
  }
