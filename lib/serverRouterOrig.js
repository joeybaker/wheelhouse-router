...
function getView(context, view, collectionName, collectionParse){
  var self = context
    , Collection = collectionName ? require(path.join(options.collections, collectionName)) : false
...
function mountRoutes(router, routes){
  _.each(routes, function(actions, pathPattern){
    _.each(actions, function(action, method){
      ...
      router.on(method, pathPatternParsed, function(){
        var self = this
          , controllerPath = path.join(options.controllers, controller)
          // , serverControllerPath = controllerPath + 'Server'
          , mutual = require(controllerPath)[fn]
          // , hasServer = fs.existsSync(serverControllerPath)
          // , server = hasServer ? require(serverControllerPath)[fn] : false
          , args = _.values(arguments)
          // , callMutual = function(){
          //   mutual.apply(self, args.concat(_.values(arguments)))
          // }
          , mutualResult = mutual.apply(this, args)

        getView.call(this, mutualResult.view, mutualResult.collection)

        // if (server) server.apply(this, args, callMutual)
        // else mutual.apply(this, args)
      })
    })
  })
}
