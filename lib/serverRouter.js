...
function buildTemplate(context, params, collection){
  var data = _.isFunction(params.data) ? params.data.call(context, collection) : params.data
    , bootstrap = _.isFunction(params.bootstrap) ? params.data.call(context, collection) : params.bootstrap ? params.bootstrap : data
    , sendRes = function(){
      context.res.end(options.render(params.view, data, {
        _data: bootstrap
        , title: params.title
        , meta: params.meta
      }))
