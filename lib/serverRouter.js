...
function buildTemplate(context, params, collection){
  var opts = _.defaults((params || {}), {
    ...
    , user = _.has(context.req, 'user')
      ...
      : undefined
    , sendRes = function(){
      var body = options.render(opts.template || opts.view, _.extend(data || {}, (user ? {_user: user} : {})), {
          _data: bootstrap
          , title: opts.title
          , meta: opts.meta
          , user: user
...
function render(context, params){
  ...
  if (!params) throw new Error('render requires options')

  if (app.collections && app.collections[params.collection]) buildTemplate(self, params, app.collections[params.collection])
  ...
  _.each(routes, function(processes, pathPattern){
    ...
    _.each(processes, function(controllerAction, method){
      ...
      app.router.on(mutual === true ? 'get' : method, pathPatternParsed, function(){
        ...
        if (!options.permissions.call(this, method, pathPattern)) {
          app.log.warn('router: permission:', this.req.method + ' permission denied for ' + this.req.url + (this.req.user ? ' for ' + this.req.user.get('email') : ''))
          this.res.writeHead(403, { 'Content-Type': 'text/html'} )
          this.res.end(options.render(options.err[403]))
          return false
        }
  ...
  app.router.configure({
    ...
    , notfound: function(err) {
      if (err) {
        ...
        this.res.end(options.render(options.err[404]))
      }
    }
    , strict: false
    , recurse: 'forward'
