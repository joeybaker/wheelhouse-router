...
function render(context, params){
  ...
  if (params && app.collections && app.collections[params.collection]) buildTemplate(self, params, app.collections[params.collection])
  ...
  _.each(routes, function(processes, pathPattern){
    ...
    _.each(processes, function(controllerAction, method){
      ...
      app.router.on(mutual === true ? 'get' : method, pathPatternParsed, function(){
        ...
        if (_.isFunction(options.permissions) && !options.permissions.call(this, method, pathPattern)) {
