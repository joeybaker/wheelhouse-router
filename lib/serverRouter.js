...
internals.respondWithError = function respondWithError(status, meta){
  ...
  // bail if the headers have already been sent b/c the user has overriden our defaults
  if (this.res.headerSent) {
    app.log.debug('router: headers already sent; not sending err', {
      status: status
      , meta: meta
    })
    return
  ...
  if (!meta.stack && /^5[0-9]{2}/.test(status)) meta.stack = new Error().stack.split('\n')
  ...
  if (internals.options.err[status]){
    this.res.end(internals.buildTemplate(this, {
      template: internals.options.err[status]
      , title: status
    }))
  }
  else
...
internals.buildTemplate = function buildTemplate(context, params, collection){
  ...
  bootstrap = opts.bootstrap === false
    ? false
    : internals.parseControllerMethod.call(context, opts, 'bootstrap', collection, model)
  user = internals.getRequestUserJSON.call(context)

  return internals.options.render.call(
    context
    , opts.template || opts.view
    , _.extend(data || {}, (user ? {_user: user} : {}))
    , {
      _data: bootstrap
      // title is not called with the context b/c it's used client and server side
      , title: _.isFunction(opts.title)
        ? opts.title(modelObject || collection)
        : opts.title
      , meta: opts.meta
      , user: user
    })
...
internals.render = function render(context, params){
  ...
  if (params && app.collections && app.collections[params.collection])
    ...
    Collection = (params && params.collection)
      ? require(path.join(internals.options.collections || '', params.collection || ''))
      : false
  ...
  _.each(routes, function(processes, pathPattern){
    ...
    _.each(processes, function(controllerAction, method){
      ...
      app.router.on(mutual === true ? 'get' : method, pathPatternParsed, function(){
        ...
        if (_.isFunction(internals.options.permissions)
          && !internals.options.permissions.call(this, method, pathPattern)){
        ...
        // we're actually dealing with a redirect here.
          ...
          this.res.redirect(controllerAction)
...
exports.attach = function(opts){
  ...
  app.router.configure({
    before: [
      function(){
        var user = _.has(this.req, 'isAuthenticated')
          && _.has(this.req, 'user')
